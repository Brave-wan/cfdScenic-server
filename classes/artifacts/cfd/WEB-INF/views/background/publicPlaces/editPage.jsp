...
<div style="width:98.5%;padding:10px;">
    <div style="padding:8px 2px;border-bottom:1px solid #ccc">当前位置:公共场所管理-修改公共场所</div>
    <form id="ff" action="./background/publicPlaces/savePublicPlaces" method="post" enctype="multipart/form-data">
        <input type="hidden" name="id" value="${model.id }"/>
        <table>
            <tr>
                <td>公共场所名称：</td>
                <td><input type="text" name="name" value="${model.name}" id="name"/>
                </td>
            </tr>

            <tr>
                <td>类型：</td>
				<td><select id="type" class=”easyui-combobox” name="type" style="width: 200px;">
						<option <c:if test="${model.type == 1 }" >selected = selected</c:if> value="1">公共厕所</option>
						<option <c:if test="${model.type == 2 }" >selected = selected</c:if> value="2">停车场</option>
				</select>
				</td>
			</tr>
			<tr>
                <td>使用状态：</td>
				<td><select id="state" class=”easyui-combobox” name="state" style="width: 200px;">
						<option value="0" <c:if test="${model.type == 1 }" >selected = selected</c:if>>正常</option>
						<option value="1" <c:if test="${model.type == 1 }" >selected = selected</c:if>>停用</option>
				</select>
				</td>
            </tr>
            <tr>
