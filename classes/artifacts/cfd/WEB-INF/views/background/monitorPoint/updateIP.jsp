...
<style type="text/css">
        ...
        <div style="padding:8px 2px;border-bottom:1px solid #ccc">当前位置:平台管理-添加新闻须知</div>
            <table class="tb">
                <tr>
                    <td>监控ip地址：</td>
                    <td>
                    	<input type="text" name="ip" id="ip" value="${model.ip}"/>
                    </td>
                </tr>
                <tr>
                    <td>帐号：</td>
                ...
                <tr>
                    <td>端口号：</td>
                    <td>
                    	<input type="text" name="port_number" id="port_number" value="${model.port_number}"/>
                    </td>
                </tr>
            </table>
            <div style="text-align:center;padding:5px">
                <a id="showPageSaveBtn" href="javascript:void(0)" onclick="updateIP()" class="easyui-linkbutton" iconCls="icon-save">确定</a>
    ...
    <script>
    	function updateIP(){
    		...
    		$.post("./monitorPoint/updateIP",{ip:ipInfo,user:user,password:password,port_number:port_number},function(date){
    			if(date.success){
    				location.reload();
    			}
    		})
