...
<style type="text/css">
    ...
    <div style="width:98.5%;padding:10px;">
        <div style="padding:8px 2px;border-bottom:1px solid #ccc">当前位置:平台管理-修改关于我们</div>
        <form id="ff" action="./htmlText/addHtmlText" method="post">
            <table>
                <tr>
                    <td>名称：</td>
                    <td>
                    	<input type="text" name="name" value="${model.name}" id="name" disabled="disabled"/>
                ...
                <tr>
                    <td>状态：</td>
                    <td>
                        <input type="radio" <c:if test="${model.state == 0}"> checked="checked"</c:if> disabled="disabled" name="state" value="0">正常
                        <input type="radio" <c:if test="${model.state == 1}"> checked="checked"</c:if> disabled="disabled" name="state" value="1">停用
                    </td>
                </tr>
                <tr>
                    <td>详情：</td>
                    <td>
                        ...
                        <input type="hidden" name="contentId" id="contentId" value="${model.contentHtml}"/>
                        
                    </td>
                </tr>
            </table>
            <div style="text-align:center;padding:5px">
               <!--  <a id="showPageSaveBtn" href="javascript:void(0)" onclick="save()" class="easyui-linkbutton" iconCls="icon-save"></a> -->
...
<script type="text/javascript">
    ...
    $(function () {
        UE.getEditor('editor').addListener("ready", function () {
            UE.getEditor('editor').setContent('${model.contentHtml}');
        })
    })
	//获取内容
    function getAllHtml() {
	...
	var id = $("#id").val();
        ...
        $.post("${basePath}/htmlText/addHtmlText",{
        	...
        	alert(data.success);
            if (data.success) {
            	 window.location.href = "./aboutus/tolist";
            } else {
                eu.showMsg(data);
            }
        }).error(function (data) {
