		...
		<div style="padding:8px 2px;border-bottom:1px solid #ccc">当前位置:订单管理-查看详情</div>
			<form id="ff" action="" method="post">
				...
				<table>
					...
					<tr>
						<td>订单描述：</td>
						<td><input type="text" value="${model.orderDescribe }" disabled="disabled"/></td>
					</tr>
					<tr>
						<td>原价：</td>
					...
					<tr>
						<td>就餐时间：</td>
						<td><input type="text" value="${model.eatDate }" disabled="disabled"/></td>
					</tr>
					<tr>
						<td>数量：</td>
					...
					<tr>
						...
						<td>
						<input type="text" 
						<c:if test="${model.payWay == 0}"> value="未支付" </c:if>
						<c:if test="${model.payWay == 1}"> value="余额" </c:if>
						<c:if test="${model.payWay == 2}"> value="支付宝" </c:if>
						<c:if test="${model.payWay == 3}"> value="微信" </c:if> disabled="disabled"/>
						</td>
					</tr>
					<tr>
						<td>支付状态：</td>
						<td>
						<input type="text" 
						<c:if test="${model.payState == 0}"> value="未支付" </c:if>
						<c:if test="${model.payState == 1}"> value="已支付" </c:if> disabled="disabled"/>
						</td>
					</tr>
					<tr>
						<td>订单状态：</td>
						<td>
						<input type="text" 
						<c:if test="${model.orderState == 1}"> value="确认订单" </c:if>
						<c:if test="${model.orderState == 2}"> value="未使用" </c:if>
						<c:if test="${model.orderState == 3}"> value="申请退款" </c:if>
						<c:if test="${model.orderState == 4}"> value="已使用" </c:if>
						<c:if test="${model.orderState == 5}"> value="已过期" </c:if>
						<c:if test="${model.orderState == 6}"> value="申请退款成功" </c:if>
						<c:if test="${model.orderState == 7}"> value="申请退款失败" </c:if> disabled="disabled"/>
						</td>
					</tr>
					<tr>
						<td>订单生成时间：</td>
					...
					<tr>
						<td>支付时间：</td>
						<td><input type="text" value="${model.payTime }" disabled="disabled"/></td>
					</tr>
					<tr>
						<td>退付时间：</td>
						<td><input type="text" value="${model.refundTime }" disabled="disabled"/></td>
					</tr>
					<tr>
						<td>用户：</td>
					...
					<tr>
						<td>应付价格：</td>
						<td><input type="text" value="${model.realPrice }" disabled="disabled"/></td>
					</tr>
					<tr>
						<td>订单号：</td>
						<td><input type="text" value="${model.orderCode }" disabled="disabled"/></td>
					</tr>
					<tr>
						<td>是否评价：</td>
						<td>
						<input type="text"
						<c:if test="${model.isComment == 0}"> value="否" </c:if>
						<c:if test="${model.isComment == 1}"> value="是" </c:if> disabled="disabled"/>
						</td>
					</tr>
					<tr>
						<td>商品：</td>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text"
                                    <c:if test="${model.goodsType == 0}"> value="单品" </c:if>
                                    <c:if test="${model.goodsType == 1}"> value="套餐" </c:if> disabled="disabled"/>
                        </td>
                    </tr>
                    <tr>
                        <td>是否结算：</td>
                        <td>
                            <input type="text"
                                    <c:if test="${model.isBalance == 0}"> value="未结算" </c:if>
                                    <c:if test="${model.isBalance == 1}"> value="已结算" </c:if> disabled="disabled"/>
                        </td>
                    </tr>
					<tr>
						<td>是否删除：</td>
						<td>
                            <input type="text"
                                    <c:if test="${model.isDelete == 0}"> value="否" </c:if>
                                    <c:if test="${model.isDelete == 1}"> value="是" </c:if> disabled="disabled"/>
						</td>
					</tr>
