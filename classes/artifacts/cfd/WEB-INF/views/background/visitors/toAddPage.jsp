...
<div style="width: 98.5%; padding: 10px;">
	...
	<form id="ff" action="./background/visitorsManage/addVisitors"
		...
		<table class="tb">
			...
			<tr>
				...
				<td><input type="text" id="nameEn" name="nameEn"
					placeholder="请输入英文名称" /></td>
			</tr>
			<tr>
				<td>简介：</td>
				<td><input type="text" id="visitorsDescribe"
					name="visitorsDescribe" placeholder="请输入简介" /></td>
			</tr>
			<c:if test="${type==1 or type==2 or type==3}">
			<tr>
				<td>原价：</td>
			...
			<tr>
				<td>折后价：</td>
				<td><input type="text" id="newPrice" name="newPrice"
					onkeyup="this.value=this.value.replace(/[^0-9]/g,'')"
					onafterpaste="this.value=this.value.replace(/[^0-9]/g,'')"
					placeholder="请输入折后价" /></td>
			</tr>
			</c:if>
			<c:if test="${type==5 or type==4}">
			...
			<tr>
				...
				<td><input type="text" id="longitude" name="longitude"
					onMouseUp="latAndLong()" readonly placeholder="请获取经度" /></td>
				<td>
					<div class="latAndLong" style="display: none">
						<img style="width: auto; height: auto"
							src="${ctx}/images/dontUpdate.png">
					</div>
				</td>
			</tr>
			<tr>
				...
				<td><input type="text" name="latitude" id="latitude"
					onMouseUp="latAndLong()" readonly placeholder="请获取纬度" /></td>
				<td>
					<div class="latAndLong" style="display: none">
						<img style="width: auto; height: auto"
							src="${ctx}/images/dontUpdate.png">
			...
			<!-- <tr>
				<td>须知：</td>
				<td><input type="text" id="noticeId" name="noticeId" /></td>
			</tr> -->
			<tr>
				<td>备注：</td>
			...
			<c:if test="${type==4 or type ==5}">
				<tr>
					<td>积分：</td>
					<td><input type="text" id="integral" name="integral"
						placeholder="请输入积分" /></td>
				</tr>
			</c:if>
			<tr>
				<td>配送费：</td>
				<td><input type="text" id="deliverFee" name="deliverFee"
					onkeyup="this.value=this.value.replace(/[^0-9]/g,'')"
					onafterpaste="this.value=this.value.replace(/[^0-9]/g,'')" placeholder="请输入配送费" /></td>
			</tr>
			<c:if test="${type==1 or type==2 or type==3 or type==5}">
				<tr>
					<td>人数：</td>
					<td><input type="text" id="number" name="number" placeholder="请输入人数" /></td>
				</tr>
			</c:if>
			<c:if test="${type==4}">
			...
			<tr>
				<td>满意度：</td>
				<td><select id="select" name="satisfaction">
			...
			<tr>
				<td>类型：</td>
				<td><select name="type" disabled id="typeId">
			...
			<tr>
				<td>是否推荐：</td>
				<td><input type="radio" id="isRecommend" checked="checked"
					name="isRecommend" value="0">正常 <input type="radio"
					id="isRecommend" name="isRecommend" value="1">热门
					<c:if test="${type==1}">
					<input type="radio"
					id="isRecommend" name="isRecommend" value="2">推荐
					</c:if>
					</td>
			</tr>
			<tr>
				<td>状态：</td>
				<td><input type="radio" id="state" checked="checked"
					name="state" value="0">正常 <input type="radio" id="state"
					name="state" value="1">删除</td>
			</tr>
			 <c:if test="${type==1 or type==2 or type==3 or type==5}">
            <tr>
                <td>开始有效期：</td>
			...
			<tr>
				<td>须知：</td>
				<td><script id="editor1" type="text/plain"
						style="width: 1024px; height: 500px;"></script> <input
					type="hidden" name="noticeId" id="noticeId" /></td>
			</tr>
			<tr>
				<td>景区详情：</td>
				<td><script id="editor2" type="text/plain"
						style="width: 1024px; height: 500px;"></script> <input
					type="hidden" name="detailsId" id="detailsId" /></td>
			</tr>
			
		</table>
		<div style="text-align: center; padding: 5px">
			<a id="showPageSaveBtn" href="javascript:void(0)" onclick="save()"
				class="easyui-linkbutton" iconCls="icon-save">确定</a>
			...
			<a id="showPageCancleBtn" href="javascript:void(0)"
...
//             if($(this).val()==""){
//                 $(this).parents(".uploader").find(".filename1").val("请选择图片...");
//             }
//         });
...
//     });
  ...
  //实例化编辑器
    ...
    //获取内容
    function getAllHtml() {
        alert(UE.getEditor('editor2').getContent())
    }

    //获取纯文本内容
    function getContentTxt() {
        var arr2 = [];
        arr2.push("使用editor.getContentTxt()方法可以获得编辑器的纯文本内容");
        arr2.push("编辑器的纯文本内容为：");
