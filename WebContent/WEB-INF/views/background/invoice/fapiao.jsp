...
<style type="text/css">
    ...
    <div style="width:98.5%;padding:10px;">
        ...
        <form id="form" action="" method="post" >
            <input type="hidden" name="invoiceState" value="${model.invoiceState }"/>
            <input type="hidden" name="linkId" value="${model.linkId}"/>
            <input type="hidden" name="type" value="${model.type }"/>
            <input type="hidden" name="invoiceTime" value="${model.invoiceTime}" />
            <input type="hidden" name="createTime" value="${model.createTime}" />
            <input type="hidden" name="id" value="${model.id}" />
            <table>
                <tr>
                    <td>付款单位名称：</td>
                    <td><input type="text" name="payUnitName" id="payUnitName" value="${model.payUnitName}"/></td>
                </tr>
                  <tr>
                    <td>税号：</td>
                    <td><input type="text" name="einNumber"id="einNumber" value="${model.einNumber}"/></td>
                </tr>
                 <tr>
                    <td>发票号：</td>
                    <td><input type="text" name="invoiceNumber" id="invoiceNumber" value="${model.invoiceNumber}"/></td>
                </tr>
                 <tr>
                    <td>开票金额：</td>
                    <td><input type="text" name="invoiceMoney" id="invoiceMoney" value="${model.invoiceMoney}"/></td>
                </tr>
                 <tr>
                    <td>订单号：</td>
                    <td><input type="text" name="orderCode" value="${model.orderCode}" readonly="readonly"/></td>
                </tr>
            </table>
...
<script type="text/javascript">
   function save(){
	   ...
	   $.post("./invoice/update",$("#form").serialize())
	   .success(function(data){
			if(data.success){
				alert("开票成功");
				window.location.href = "./invoice/tolist";
			} else {
				alert(data);
			}
		}).error(function(data){
