...
<div style="width:98.5%;padding:10px;">
		<div style="padding:8px 2px;border-bottom:1px solid #ccc">当前位置:系统管理-修改轮播图</div>
			<form id="ff" action="./background/advertisement/insert" method="post" enctype="multipart/form-data">
				...
				<table class="tb">
					...
					<tr>
						...
						<td>
                            <select name="type">
                                <option <c:if test="${model.type==0}">selected</c:if> value="0">轮播图</option>
                                <option <c:if test="${model.type==1}">selected</c:if> value="1">广告位</option>
                                <option <c:if test="${model.type==2}">selected</c:if> value="2">商品图片</option>
                            </select>
                        </td>
					</tr>
					<tr>
						<td>播图跳转类型：</td>
						<td>
                            <select name="advertisementType" id="advertisementType">
                                <option value="1" <c:if test="${model.advertisementType==1}">selected</c:if> >酒店</option>
                                <option value="2" <c:if test="${model.advertisementType==2}">selected</c:if> >饭店</option>
                                <option value="3" <c:if test="${model.advertisementType==3}">selected</c:if> >特产</option>
                                <option value="4" <c:if test="${model.advertisementType==4}">selected</c:if> >小吃</option>
                                <option value="5" <c:if test="${model.advertisementType==5}">selected</c:if> >景点详情</option>
                                <option value="6" <c:if test="${model.advertisementType==6}">selected</c:if> >活动详情</option>
                                <option value="7" <c:if test="${model.advertisementType==7}">selected</c:if> >观鸟景点</option>
                                <option value="8" <c:if test="${model.advertisementType==8}">selected</c:if> >湿地保护</option>
                                <option value="9" <c:if test="${model.advertisementType==9}">selected</c:if> >招商信息</option>
                            </select>
                        </td>
					</tr>
					<tr>
						<td>跳转地址：</td>
						<td>
						<input type="hidden" id="linkId" value="${model.linkId}"/>
						<select id="select" name="linkId" value="${model.linkId}">
							<c:forEach items="${list}" var="vs">
								<option <c:if test="${model.linkId == vs.id}">selected</c:if> value="${vs.id}">${vs.name}</option>
							</c:forEach>
						</select>
						</td>
					</tr>
                    <tr>
                        <td>状态：</td>
                        <td>
                            <input type="radio" name="state" <c:if test="${model.state==0}">checked</c:if> value="0">可用
                            <input type="radio" name="state" <c:if test="${model.state==1}">checked</c:if> value="1">不可用
                        </td>
                    </tr>
                    <tr>
                        <td>备注：</td>
                    ...
                    <tr>
                        <td>图片：</td>
                        <td id="img"><img  src="${model.imgUrl}"></td>
                    </tr>
                    <tr>
                        <td>修改图片：</td>
...
$("#advertisementType").change(function(){
	...
	$.post("./background/advertisement/getJumpType",{type:advertisementType},function(data){
		if(data.success){
			$("#select").empty(); 
			...
			for(var i=0;i<data.obj.length;i++){
				if(data.obj[i].id == $("#linkId").val()){
					$("#select").append("<option selected value="+data.obj[i].id+">"+data.obj[i].name+"</option>");
				}else{
