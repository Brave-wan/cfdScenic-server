...
<div style="width:98.5%;padding:10px;">
		<div style="padding:8px 2px;border-bottom:1px solid #ccc">当前位置:门票管理-查看详情</div>
			<form id="ff" action="" method="post">
				...
				<table>
					...
					<tr>
						<td>开始有效期：</td>
						<td><input type="text" value="${model.start_valid }" disabled="disabled"/></td>
					</tr>
					<tr>
						<td>结束有效期：</td>
						<td><input type="text" value="${model.end_valid }" disabled="disabled"/></td>
					</tr>
					<tr>
						<td>数量：</td>
					...
					<tr>
						...
						<c:if test="${model.order_state == 9}"> value="作废" </c:if> disabled="disabled"/>
						</td>
					</tr>
					<tr>
						<td>订单生成时间：</td>
						<td><input type="text" value="${model.create_time }" disabled="disabled"/></td>
					</tr>
					<tr>
						<td>支付时间：</td>
						<td><input type="text" value="${model.pay_time }" disabled="disabled"/></td>
					</tr>
					<tr>
						<td>退付时间：</td>
					...
					<tr>
						<td>用户：</td>
						<td><input type="text" value="${model.user_name }" disabled="disabled"/></td>
					</tr>
					<tr>
						<td>应付价格：</td>
					...
					<tr>
						<td>景点：</td>
						<td><input type="text" value="${model.v_name }" disabled="disabled"/></td>
					</tr>
					<tr>
						<td>配送地址：</td>
					...
					<tr>
						...
						<td>
						<input type="text" 
						<c:if test="${model.is_mention == 0}"> value="否" </c:if>
						<c:if test="${model.is_mention == 1}"> value="是" </c:if> disabled="disabled"/>
						</td>
					</tr>
					<tr>
						<td>是否删除：</td>
					...
					<tr>
						...
						<td>
						<input type="text" 
						<c:if test="${model.is_invoice == 0}"> value="否" </c:if>
						<c:if test="${model.is_invoice == 1}"> value="是" </c:if> disabled="disabled"/>
						</td>
					</tr>
					<tr>
						<td>发票状态：</td>
				...
				<div style="text-align:center;padding:5px">
					...
					<a id="showPageCancleBtn" href="javascript:void(0)" onclick="window.location.href ='./visitorsOrder/toVisitorsOrderWei'" class="easyui-linkbutton" iconCls="icon-save">确定</a>
			    </div>
			</form>
</div>
